...
const roleLayers = {
  text: '04 Labels and dimensions',
  stroke: '05 Study strokes',
  fill: '07 Study fills',
  guide: '06 Construction guides',
  grid: '08 Coordinate grids - 2px',
};
...
function fixture() {
  const doc = {
    ...
    groupItems: [],
    saved: true,
    fullName: { fsName: '/fixture.ai' },
    saveCalls: [],
    saveAs(file) {
      this.saveCalls.push(file.fsName);
      this.fullName = file;
      this.saved = true;
    },
  };
  ...
  function item(type, name, parent, bounds) {
    const obj = {
      ...
      get layer() {
        let p = this.parent;
        while (p.typename !== 'Layer') p = p.parent;
        return p;
      },
      get geometricBounds() {
        if (this.closed && this.pathPoints) {
          return [
            Math.min(...this.pathPoints.map((p) => p.anchor[0])),
            Math.max(...this.pathPoints.map((p) => p.anchor[1])),
            Math.max(...this.pathPoints.map((p) => p.anchor[0])),
            Math.min(...this.pathPoints.map((p) => p.anchor[1])),
          ];
        }
      ...
      translate(dx, dy) {
        ...
        while (p.typename !== 'Document') {
          assert.equal(p.locked, false, 'translation requires unlocked ancestors');
          p = p.parent;
        }
  ...
  bg.pathPoints = [
    ...
    [431, -446],
    [-1, -446],
  ].map((a) => ({ anchor: a.slice(), leftDirection: a.slice(), rightDirection: a.slice() }));
  ...
  for (let col = 0; col < 2; col++) {
    ...
    for (let row = 0; row < ids.length; row++) {
      ...
      line.closed = false;
      line.pathPoints = [{ anchor: [x, y] }, { anchor: [x + 192, y] }];
      paper.pageItems.push(line);
    }
  }
  const globalLine = item('PathItem', 'global-divider', paper, [11, 404, 419, 404]);
  globalLine.closed = false;
  globalLine.pathPoints = [{ anchor: [11, 404] }, { anchor: [419, 404] }];
  paper.pageItems.push(globalLine);
  let output,
    redraws = 0;
  ...
  function run(apply = false, allowUnsaved = false) {
    ...
    ctx.config.allowUnsaved = allowUnsaved;
    vm.runInContext(source, ctx);
    return JSON.parse(output);
  }
  function snapshot() {
    return doc.groupItems.map((g) => [g.name, g._b.slice(), g.locked]);
  }
  return {
    doc,
    layers,
...
function assertUnchangedPlan(f) {
  ...
  assert.equal(result.movedItems, 0);
  assert.equal(result.countLabels, 0);
  assert.equal(result.dividerMoves, 0);
  assert.equal(result.dividerNames, 0);
  assert.equal(result.backgroundChanged, false);
}

test('check is read-only; apply is idempotent, restores locks and redraws', () => {
  const f = fixture(),
    before = f.snapshot();
  assert(f.run().movedIcons > 0);
  assert.deepEqual(f.snapshot(), before);
  assert.equal(f.doc.saveCalls.length, 0);
  assert.equal(f.run(true).mode, 'apply');
  assert(f.redraws > 0);
  assert(Object.values(f.layers).every((l) => l.locked));
...
test('removing the first icon column preserves divider anchors and heading x', () => {
  const f = fixture();
  f.run(true);
  const heading = f.find('Geometry 01 - Test / text').pageItems.find((g) => g.name === 'title-01'),
    oldX = heading._b[0];
  for (const role of Object.keys(roleLayers))
    for (const k of [0, 4]) {
      const g = f.find(`${role}-icon-01-${k}`);
      g.parent.pageItems.splice(g.parent.pageItems.indexOf(g), 1);
      f.doc.groupItems.splice(f.doc.groupItems.indexOf(g), 1);
    }
  assert.equal(f.run(true).mode, 'apply');
  assert.equal(f.find('grid-icon-01-1').geometricBounds[0], 18);
  assert.equal(heading.geometricBounds[0], oldX);
  assert.equal(f.find('section-divider-01').geometricBounds[0], 11);
  assertUnchangedPlan(f);
});

for (const role of Object.keys(roleLayers))
  test(`duplicate ${role} is rejected before backup`, () => {
    const f = fixture(),
      original = f.find(`${role}-icon-01-0`);
    const duplicate = f.item('GroupItem', original.name, original.parent, original._b.slice());
    original.parent.pageItems.push(duplicate);
    f.doc.groupItems.push(duplicate);
    assert.match(f.run(true).error, new RegExp(`Duplicate ${role}`));
    assert.equal(f.doc.saveCalls.length, 0);
  });

test('orphan role outside its section is rejected', () => {
  const f = fixture(),
    g = f.find('guide-icon-01-0');
  g.parent.pageItems.splice(g.parent.pageItems.indexOf(g), 1);
  g.parent = f.layers.guide;
  assert.match(f.run(true).error, /Orphan study role/);
  assert.equal(f.doc.saveCalls.length, 0);
});

test('tall first-row labels clear the divider and push subsequent rows down', () => {
  const f = fixture(),
    label = f.find('text-icon-01-0');
  label._b[1] += 40;
  label._b[3] -= 80;
  assert.equal(f.run(true).mode, 'apply');
  assert(label.visibleBounds[1] <= f.find('section-divider-01').geometricBounds[1] - settings.firstRowGap + 0.001);
  const next = f.find('text-icon-01-4');
  assert(next.visibleBounds[1] <= label.visibleBounds[3] - settings.rowGap + 0.001);
  assertUnchangedPlan(f);
});
...
test('unsaved apply stops unless explicitly allowed, with live backup', () => {
  const f = fixture();
  f.doc.saved = false;
  assert.match(f.run(true).error, /--allow-unsaved/);
  assert.equal(f.doc.saveCalls.length, 0);
  assert.equal(f.run(true, true).mode, 'apply');
  assert.deepEqual(f.doc.saveCalls, ['/backup', '/fixture.ai']);
});

test('partial failure reverses only own moves and restores all layer locks', () => {
  const f = fixture(),
    before = f.snapshot(),
    target = f.find('grid-icon-03-0'),
    translate = target.translate;
  let once = true;
  target.translate = function (dx, dy) {
    if (once) {
      once = false;
      throw Error('injected move failure');
    }
...
test('rollback failure still restores lock states and retains the backup', () => {
  const f = fixture(),
    moved = f.find('text-icon-03-0'),
    translate = moved.translate;
  let calls = 0;
  moved.translate = function (dx, dy) {
    if (++calls === 2) throw Error('injected rollback failure');
    translate.call(this, dx, dy);
  };
  f.find('grid-icon-03-0').translate = function () {
    throw Error('injected move failure');
  };
  const result = f.run(true);
  assert.match(result.rollbackError, /rollback failure/);
  assert.equal(result.backup, '/backup');
  assert(Object.values(f.layers).every((l) => l.locked));
  assert.deepEqual(f.doc.saveCalls, ['/backup']);
});

function addSymbolGrid(f, withFrame = true) {
  const grid = f.find('grid-icon-01-0'),
    bounds = grid._b.slice();
...
for (const invalid of ['filled', 'stroked', 'clipping', 'open', 'size'])
  test(`invalid ${invalid} coordinate-frame stops before backup`, () => {
    const f = fixture(),
      frame = addSymbolGrid(f);
...
test('ordinary first-row labels preserve the existing header-to-grid distance', () => {
  const f = fixture(),
    before = f.find('grid-icon-01-0').geometricBounds[1];
  assert.equal(f.run(true).mode, 'apply');
