...
function createFixture(options = {}) {
  ...
  const append = (parent, item) => {
    item.parent = parent;
    parent.pageItems.push(item);
    return item;
  };
  const remove = (item) => {
    item.parent.pageItems.splice(item.parent.pageItems.indexOf(item), 1);
    if (options.strictDeletes)
      Object.defineProperty(item, 'locked', {
        get() {
          throw new Error('deleted lock read');
        },
        set() {
          throw new Error('deleted lock write');
        },
      });
  };
  ...
  const group = (name, owner) => {
    ...
    item.groupItems = {
      add() {
        return append(item, group('', owner));
      },
    };
    item.symbolItems = {
      add(definition) {
        return append(item, symbol(definition, options.failTranslate));
      },
    };
    item.remove = () => remove(item);
    return item;
  ...
  document.saveAs = (file) => {
    saves++;
    document.fullName = { fsName: file.fsName };
    document.saved = true;
  };
  document.close = () => {};
  document.activate = () => {
...

{
  ...
  expect(
    fixture.copies() === 0 && fixture.saves() === 0 && fixture.activations() === 1,
    'unsaved apply must activate before stopping without back up or save'
  );
}

{
  ...
  expect(
    part.pageItems.length === 1 && part.pageItems[0].symbol === fixture.circleSet && part.pageItems[0].name === 'Circle R0.75 + Center',
    'Circle + Center should place one readable named set instance'
  );
...

{
  ...
  expect(
    result === 'LISM_OK: checked guide part' && fixture.report().newPart === 'part-radius-0p75-at-m2p25-4p5',
    'at suffix should preserve negative and decimal coordinates'
  );
}
...

{
  ...
  expect(
    result === 'LISM_OK: placed guide part' && item.rotation === 90 && item.translatedAfterRotate === 90,
    'rotation should happen before centering'
  );
}
